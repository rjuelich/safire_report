...
do
	if [ ${project_raw} = "geno" ]
	then
		if [ `grep -c ${mri} ${study_home}/.FEP_sublist` -gt 0 ]
		then
			project="FEP"
		else
			project="Taplin_Ongur"
		fi
	else
		project=${project_raw}
	fi
	...
	vbr=`grep VBR ${study_home}/${olid}/mri/processed/fs450/${mri}/qc/INDIV_MAPS/${mri}_bsv_table.txt | awk '{printf "%.4f,%.1f\n", $2, $4}'`

	echo '<tr width="100%" style="page-break-inside: avoid;"><td width=1% align=center><a href='PDFs/${mri}.pdf' target="_blank">'${nsub}'</a></td><td width=5.5% align=center><a href="https://redcap.partners.org/redcap/redcap_v7.0.14/DataEntry/record_home.php?pid=5739&arm=1&id='${olid}'">'${olid}'</a></td><td align=center>'${vis}'</td><td width=13% align=center><a href="http://xnat.mclean.harvard.edu/data/archive/projects/'${project}'/subjects/'${olid}'/experiments/'${mri}'" target="_blank">'${mri}'</a></td>'
